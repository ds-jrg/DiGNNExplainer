#This code runs in torch-geometric 2.4.0
import argparse
import os
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train DBLP / IMDB Model")

    parser.add_argument("--data_name", nargs="?", default="imdb", help="Input data path.")
    parser.add_argument(
        "--model_path",
        nargs="?",
...
class Real_NC_GCN(torch.nn.Module):
    def __init__(self, num_unit, n_input, n_out, n_hid):
        super(Real_NC_GCN, self).__init__()
...
if __name__ == "__main__":
    ...
    model = Real_NC_GCN(args.num_unit, n_input=n_input, n_out=n_labels, n_hid=args.hidden).to(device)

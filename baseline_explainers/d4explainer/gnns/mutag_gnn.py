...
class Mutag_GCN(torch.nn.Module):
    def __init__(self, conv_unit=3):
        ...
        self.convs.append(GCNConv(in_channels=7, out_channels=128))
...
if __name__ == "__main__":
    ...
    test_dataset = Mutag(args.data_path, mode="testing")
    val_dataset = Mutag(args.data_path, mode="evaluation")
    train_dataset = Mutag(args.data_path, mode="training")
    if args.random_label:
        for dataset in [test_dataset, val_dataset, train_dataset]:
            for g in dataset:
                g.y.fill_(random.choice([0, 1]))
    #Added code to set all -1 to 0
    for dataset in [test_dataset, val_dataset, train_dataset]:
        for g in dataset:
            g.y[g.y== -1] = 0

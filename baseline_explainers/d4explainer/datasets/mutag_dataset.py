...
class Mutag(InMemoryDataset):
    ...
    def __init__(
        ...
        self.mode = mode
        super(Mutag, self).__init__(root, transform, pre_transform, pre_filter)
    ...
    def process(self):
        ...
        assert len(data_list) == 188

        random.shuffle(data_list)
        torch.save(self.collate(data_list[:150]), self.processed_paths[0])
        torch.save(self.collate(data_list[150:169]), self.processed_paths[1])
        torch.save(self.collate(data_list[169:]), self.processed_paths[2])

...
class Syn_NC_Dataset(InMemoryDataset):
    def __init__(self, root, name, mode="testing", transform=None, pre_transform=None):
        self.name = name
        self.mode = mode
        super(Syn_NC_Dataset, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        ...
        if self.name == 'BA_shapes':
            x=torch.tensor([1.0,0.0,0.0,0.0]).repeat(len(features),1)
        elif self.name in ['Tree_Cycle', 'Tree_Grids']:
            x=torch.tensor([1.0, 0.0]).repeat(len(features),1)
        y = (

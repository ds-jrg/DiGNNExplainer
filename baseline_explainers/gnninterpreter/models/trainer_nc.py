...
class Trainer:
    ...
    def train(self, iterations, dynamic_penalty=None, penalty_cond=None, break_cond=None):
        ...
        for _ in (bar := tqdm(range(int(iterations)), initial=self.iteration, total=self.iteration+iterations)):
            ...
            score_dict = {v: scores[k] for k, v in self.dataset.NODE_CLS.items()}
    ...
    def quantatitive(self, sample_size=1000, sample_fn=None):
        ...
        return dict(label=list(self.dataset.NODE_CLS.values()),
    ...
    def save(self, G, cls_idx, root="result"):
        if isinstance(cls_idx, tuple):
            path = f"{root}/{self.dataset.name}/{self.dataset.NODE_CLS[cls_idx[0]]}-{self.dataset.NODE_CLS[cls_idx[1]]}"
        else:
            path = f"{root}/{self.dataset.name}/{self.dataset.NODE_CLS[cls_idx]}"

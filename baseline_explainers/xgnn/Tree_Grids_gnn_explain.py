...
class gnn_explain():
    def __init__(self, max_node, max_step, target_class, max_iters): 
        ...
        self.gnnNets = gcn_nc.GCN(2,32,2)
    ...
    def train(self):
        ...
        checkpoint = torch.load('checkpoint/TreeGrids_gcn.pth')
                ...
                if  flag == True: #### add edge  successfully
                    if validity == True:                        
                        ...
                        if correct_indices:
                            ...
                            reward_pred = probs.ravel()[self.target_class]- 0.3 # positive reward
                        else:
                            probs = logits.softmax(dim=-1)[0]
                            reward_pred = probs[self.target_class] - 0.3 #negative reward
    ...
    def roll_out(self, graph, j):
        ...
        ### Todo
        probs = logits.softmax(dim=-1)[0]
        reward = probs[self.target_class] - 0.3
        return reward

...
class gnn_explain():
    def __init__(self, max_node, max_step, target_class, max_iters): 
        ...
        self.gnnNets = gcn_nc.GCN(3,32,3)
        ...
        self.max_poss_degree = {0: 4, 1: 4, 2: 4}
        

    def train(self):
        ####given the well-trained model
        ### Load the model
        checkpoint = torch.load('checkpoint/imdb_gcn.pth')
                ...
                if  flag == True: #### add edge  successfully
                    if validity == True:                        
                        ...
                        if correct_indices:
                            ...
                            reward_pred = probs.ravel()[self.target_class]- 0.1 # positive reward
                        else:
                            probs = logits.softmax(dim=-1)[0]
                            reward_pred = probs[self.target_class] - 0.1 #negative reward
    ...
    def roll_out(self, graph, j):
        ...
        ### Todo
        probs = logits.softmax(dim=-1)[0]
        reward = probs[self.target_class] - 0.1
        return reward

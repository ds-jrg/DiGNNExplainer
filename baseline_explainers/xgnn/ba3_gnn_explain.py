...
class gnn_explain():
    def __init__(self, max_node, max_step, target_class, max_iters): 
        ...
        self.max_poss_degree = {0: 4, 1: 5, 2: 2, 3:3}
        

    def train(self):
        ####given the well-trained model
        ### Load the model 
        checkpoint = torch.load('checkpoint/ba3_gcn.pth')
                ...
                if  flag == True: #### add edge  successfully
                    if validity == True:                        
                        ...
                        for data in data_loader:
                            #https://github.com/pyg-team/pytorch_geometric/discussions/6655
                            logits = self.gnnNets(data.x.float(),data.edge_index.to(torch.int64),batch=data.batch)
                        prediction = logits.argmax(dim=-1)
                        probs = logits.softmax(dim=-1).squeeze(0)

                           #### based on logits, define the reward
                        if self.target_class == prediction:
                            reward_pred = probs[prediction] - 0.05 # positive reward
                        else:
                            reward_pred = probs[self.target_class] - 0.05 #negative reward
    ...
    def roll_out(self, graph, j):
        ...
        probs = logits.softmax(dim=-1).squeeze(0)
        ### Todo
        reward = probs[self.target_class] - 0.05
        return reward

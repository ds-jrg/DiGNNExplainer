...
if __name__ == '__main__':
    ...
    path_adj = '../data/MUTAG/MUTAG_A.txt'
    path_labels = '../data/MUTAG/MUTAG_graph_labels.txt'
    path_graph_indicator = '../data/MUTAG/MUTAG_graph_indicator.txt'
    path_labels_node = '../data/MUTAG/MUTAG_node_labels.txt'

    base_path = pathlib.Path(os.path.realpath(__file__)).parents[0]
    path_adj = os.path.join(base_path,path_adj)
    path_labels = os.path.join(base_path, path_labels)
    path_graph_indicator = os.path.join(base_path, path_graph_indicator)
    path_labels_node = os.path.join(base_path, path_labels_node)
    ...
    for epoch in range(10):
        ...
        if patience <= 0:
            ...
            if not os.path.isdir('../checkpoint'):
                os.mkdir('../checkpoint')
            torch.save(state, '../checkpoint/mutag_gcn.pth')
            break

...
class ExtraFeatures:
    def __init__(self, extra_features_type, dataset_name, dataset_info):
        self.max_n_nodes = dataset_info.max_n_nodes
        self.ncycles = NodeCycleFeatures(dataset_name)
...
class NodeCycleFeatures:
    def __init__(self,dataset_name):
        self.kcycles = KNodeCycles(dataset_name)
...
class KNodeCycles:
    ...
    def k_cycles(self, adj_matrix, verbose=False):
        ...
        #     kcyclesx = torch.cat([k3x, k5x], dim=-1)
        #     kcyclesy = torch.cat([k3y, k5y, k6y], dim=-1)

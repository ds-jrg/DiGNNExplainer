...
class ChargeFeature:
    ...
    def __call__(self, noisy_data):
        ...
        bond_orders = torch.tensor([1, 2, 3, 1.5], device=noisy_data['E_t'].device).reshape(1, 1, 1, -1)
...
class ValencyFeature:
    ...
    def __call__(self, noisy_data):
        ...
        orders = torch.tensor([1, 2, 3, 1.5], device=noisy_data['E_t'].device).reshape(1, 1, 1, -1)
